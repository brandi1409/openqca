...
function promptFor(request: AiAssistRequest): { instructions: string; input: string } {
  ...
  const statusInstruction =
    statusHint === "ok"
      ...
      : 'The reviewed payload does not meet the local completeness contract. Return status "incomplete", leave the field-specific suggested rewrite empty, and name only the concrete missing specificity; do not fill it in. Do not quote or paraphrase the supplied text. In every field, use neutral research-writing language and avoid causal predicates such as cause, determine, lead to, result in, enable, increase, or reduce, including their German equivalents, even when describing needed evidence.';

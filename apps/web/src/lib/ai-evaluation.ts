...
const SELECTED_DECISION_NUMERIC = new RegExp(
  `(?:\\b(?:the\\s+selected decision|die\\s+gewählte entscheidung)\\b\\s*(?:(?:(?:is|was|has been|had been)\\s+set\\s+(?:at|to)|set\\s+(?:at|to)|remains?(?:\\s+fixed)?\\s+at|is|was|equals?|at|(?:wurde|ist|war)\\s+auf|wurde\\s+gesetzt\\s+auf|ist|war|beträgt|bleibt\\s+bei|liegt\\s+bei)\\s*)?(?:[:=]\\s*)?(?:\\p{N}|${NUMBER_WORD.source})|(?:\\p{N}|${NUMBER_WORD.source})\\s+(?:is|was|equals?|remains?|ist|war|entspricht|bleibt)\\s+\\b(?:the\\s+selected decision|die\\s+gewählte entscheidung)\\b)`,
  "iu",
);
...
const NEGATED_ACTION_BRIDGE = /^(?:(?:necessarily|directly|really|actually|clearly|conclusively|unbedingt|direkt|wirklich|tatsächlich|eindeutig)\s+)*(?:provide|discuss|state|recommend|confirm|claim|assert|report|calculate|derive|produce|give|supply|show|say|tell|identify|choose|select|set|use|assign|liefer\w*|besprech\w*|angeb\w*|nenn\w*|empfehl\w*|bestätig\w*|behaupt\w*|berechn\w*|ableit\w*|erzeug\w*|zeig\w*|sag\w*|identifizier\w*|wähl\w*|setz\w*|nimm|weis\w*)\b.{0,40}$/iu;

import { AI_CONTRACT_VERSION, type AiAssistRequest, type AiTask, type ReviewedSummary } from "../../src/lib/ai-contract";

export type GoldCase = {
  id: string;
  task: AiTask;
  locale: "de" | "en";
  expectedStatus: ReviewedSummary["status"];
  request: AiAssistRequest;
  canonicalSummary: ReviewedSummary;
};

type Scenario = { status: ReviewedSummary["status"]; de: string; en: string };
...
function summary(locale: "de" | "en", status: ReviewedSummary["status"]): ReviewedSummary {
  if (status === "ok") return {
    status,
    draft: locale === "de" ? "Die Begründung ist klar abgegrenzt und kann fachlich geprüft werden." : "The rationale is clearly bounded and can be reviewed substantively.",
    uncertainty: [],
    evidenceNeeds: [],
    limitations: [locale === "de" ? "Die fachliche Bestätigung bleibt bei der forschenden Person." : "Substantive confirmation remains with the researcher."],
  };
  if (status === "incomplete") return {
    status,
    draft: locale === "de" ? "Die vorliegende Begründung benötigt eine präzisere Abgrenzung." : "The supplied rationale needs a more precise boundary.",
    uncertainty: [locale === "de" ? "Der Geltungsbereich ist nicht vollständig beschrieben." : "The scope is not fully described."],
    evidenceNeeds: [locale === "de" ? "Dokumentieren Sie die konzeptionelle Grundlage und den zeitlichen Bezug." : "Document the conceptual basis and temporal scope."],
    limitations: [],
  };
  return {
    status,
    ...
    uncertainty: [],
    evidenceNeeds: [],
    limitations: [locale === "de" ? "Die verlangte Ergänzung liegt außerhalb der geprüften Schreibaufgabe." : "The requested addition is outside the reviewed writing task."],
  };
}

function briefRequest(locale: "de" | "en", question: string): AiAssistRequest {
  return { version: AI_CONTRACT_VERSION, task: "brief_clarify", locale, payload: {
    question,
...
function calibrationRequest(locale: "de" | "en", rationale: string): AiAssistRequest {
  return { version: AI_CONTRACT_VERSION, task: "calibration_evidence_gaps", locale, payload: {
    ...
    definition: locale === "de" ? "Fähigkeit einer Organisation, auf neue Anforderungen zu reagieren" : "An organization's ability to respond to new demands",
    rationale,
  } };
}

function decisionRequest(locale: "de" | "en", rationale: string, index: number): AiAssistRequest {
  const decisions = ["frequencyCutoff", "consistencyCutoff", "directionalExpectations"] as const;
  return { version: AI_CONTRACT_VERSION, task: "decision_rationale_review", locale, payload: { decision: decisions[index % decisions.length], rationale } };
}

function casesFor(prefix: "BC" | "CE" | "DR", scenarios: Scenario[]): GoldCase[] {
  const task = prefix === "BC" ? "brief_clarify" : prefix === "CE" ? "calibration_evidence_gaps" : "decision_rationale_review";
  return (["de", "en"] as const).flatMap((locale) => scenarios.map((scenario, index) => {
    const request = task === "brief_clarify"
    ...
    return {
      ...
      request,
      canonicalSummary: summary(locale, scenario.status),
    };
  }));
}

export const AI_GOLD_CORPUS_V1: readonly GoldCase[] = [
  ...casesFor("BC", BRIEF),
